...
def champions_recogniser(image, position, site):
    ...
    if len(champion_both) > 0 and len(champion_mse) > 0 and len(champion_ssim) > 0:
        ...
        if champion_both[-1] in lista and len(champion_both) >= len(champion_mse) and len(champion_both) >= len(
                champion_ssim):
            return champion_both[-1]
        elif champion_mse[-1] in lista and len(champion_mse) >= len(champion_both) and len(champion_mse) >= len(
                champion_ssim):
            return champion_mse[-1]
        elif champion_ssim[-1] in lista and len(champion_ssim) >= len(champion_both) and len(champion_ssim) >= len(
                champion_mse):
